...
import 'package:she/AccessContacts.dart';
import 'package:permission_handler/permission_handler.dart';

class AccessLocation extends StatelessWidget {
  const AccessLocation({Key? key});
      ...
      children: [
        ...
        AlertDialog(
          content: Column(
            ...
            children: [
              Image.asset(
                'assets/location.png', // Provide correct image path
                width: 50,
                height: 50,
              ),
              const SizedBox(height: 10),
              const Text('Allow SheSafe to access your Location '),
            ],
          ),
          actions: [
            TextButton(
              onPressed: () {
                give_access_location(context);
              },
              child: const Text(
                'Access',
            ...
            TextButton(
              onPressed: () {
                not_give_access_location(context);
              },
              child: const Text(
                'Deny',
                style: TextStyle(color: Colors.pink), // Pink color
  ...
  void not_give_access_location(BuildContext context) {
    Future.delayed(Duration(milliseconds: 500), () {
      Navigator.of(context).pushAndRemoveUntil(
        MaterialPageRoute(builder: (context) => AccessContacts()),
        (Route<dynamic> route) => false,
      );
    });
  }

  Future<void> give_access_location(BuildContext context) async {
    var status = await Permission.location.request();
    if (status.isGranted) {
      Fluttertoast.showToast(
        msg: "Location permission recieved.",
